...
torchrun --nproc_per_node=1 --master_port=$((20000 + RANDOM % 20000)) main_finetune.py \
  ...
  --global_pool --batch_size 128 --nb_classes 3 \
  --data_path $PROJECT/retfound/data/neh_sets/${NEHSET} --input_size 224 \
  --task neh_${NEHSET}_${SRC} \
  --output_dir $PROJECT/retfound/outputs \
  --adaptation lp --eval \
  --resume $PROJECT/retfound/outputs/${SRC}/checkpoint-best.pth
